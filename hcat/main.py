from hcat.utils import calculate_indexes, remove_edge_cells, remove_wrong_sized_cells
from hcat.cell import Cell
import hcat.utils as utils
from hcat.cochlea import Cochlea
...
import numpy as np
from hcat.explore_lif import get_xml
...
def analyze(f: str, channel: int, render: bool,
    ...
    with torch.no_grad():
        ...
        for (x, y) in tqdm(product(x_ind, y_ind), total=total):
            ...
            out = backend(image.to(device)) if unet else backend(image[:, [channel], ...].to(device))

...
class Serie(SerieHeader):
    ...
    def getFrame(self, T=0, channel=0, dtype=np.uint16):
        ...
        Leica use uint8 by default, but after deconvolution the datatype is np.uint16 NOT ALWAYS TRUE - but maybe true enough???

        """
        zyx = np.zeros(self.getFrameShape(), dtype='uint16')

...
class r_unet(nn.Module):
    ...
    def __init__(self, in_channels:  int = 1, n: int = 5, c: List[int] = [20, 25, 30]):
        ...
        self.down_1 = r_block(in_channels, c[0], n, 1, padding=3)
        self.down_2 = r_block(c[0], c[1], n, 1, padding=3)
        self.down_3 = r_block(c[1], c[2], n, 1, padding=3)
...
class r_block(nn.Module):
    ...
    """
    def __init__(self, in_channels: int, out_channels: int, n_loop: int, dilation: int = 1, padding: int = 3):
        ...
        self.conv7x7_1 = nn.Conv3d(in_channels, in_channels, kernel_size=7, padding=padding, stride=1, dilation=dilation)
        self.conv7x7_2 = nn.Conv3d(in_channels, out_channels, kernel_size=7, padding=padding, stride=1, dilation=dilation)
